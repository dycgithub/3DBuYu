...
{
    /// <summary>
    /// 集中式子弹模拟器。敌人仍然是 MonoBehaviour，Flocking ECS 不受影响。
    /// </summary>
    public sealed class ProjectileSimulationService : ITickable, IDisposable
    {
        ...
        private readonly IDamageApplier _damagePipeline;
        private readonly IBulletEffectDispatcher _effectDispatcher;
        ...
        public ProjectileSimulationService(
            ...
            IDamageApplier damagePipeline,
            IBulletEffectDispatcher effectDispatcher)
        {
            _pool = pool;
        ...
        {
            return TrySpawnBatch(in info, 1);
        }
        ...
        private void SpawnSingle(in ProjectileInfo info)
        {
            ...
            if (info.Profile.Trajectory != null && info.Profile.Trajectory.IsHitscan)
            {
            ...
            GameObject view = _pool.Rent(normalized.Profile, normalized.Origin, rotation);
        ...
        private void Simulate(ProjectileRuntime projectile, float deltaTime)
        {
            ...
            Vector3 direction = projectile.Info.Profile.Trajectory != null
                ? projectile.Info.Profile.Trajectory.GetDirection(
                    projectile,
                    projectile.Direction).normalized
                : projectile.Direction;
            if (direction.sqrMagnitude < 0.0001f)
                direction = projectile.Direction.sqrMagnitude > 0.0001f
                    ? projectile.Direction.normalized
                    : Vector3.forward;
            projectile.Direction = direction;
            if (_hitQuery.TrySweep(projectile, travelDistance, out RaycastHit hit, out IDamageable target))
            {
            ...
            {
                projectile.Position = start + direction * travelDistance;
            ...
            if (!projectile.IsComplete &&
                (projectile.TraveledDistance >= projectile.Info.MaxDistance ||
                 projectile.RemainingLife <= 0f))
            {
                CompleteExpired(projectile);
            }
        }
        ...
        {
            if (!_hitQuery.TryRaycast(info, out RaycastHit hit, out IDamageable target))
            {
                _effectDispatcher.DispatchExpired(info.Profile, info.Origin + info.Direction * info.MaxDistance);
                return;
            }

            DamageRequest request = CreateDamageRequest(info, hit.point, hit.normal);
            _effectDispatcher.DispatchTrigger(info.Profile, request, target);
            _damagePipeline.TryApply(target, request, out DamageResult result);
            _effectDispatcher.DispatchHit(info.Profile, request, result, target);
        }
            ...
            Vector3 hitNormal)
        {
            ...
            _effectDispatcher.DispatchTrigger(projectile.Info.Profile, request, target);
            _damagePipeline.TryApply(target, request, out DamageResult result);
            _effectDispatcher.DispatchHit(projectile.Info.Profile, request, result, target);
        ...
        private void CompleteExpired(ProjectileRuntime projectile)
        {
            ...
            projectile.IsComplete = true;
            _effectDispatcher.DispatchExpired(projectile.Info.Profile, projectile.Position);
        }

        private void UpdateView(ProjectileRuntime projectile)
        {
        ...
        {
            _pool.Return(projectile.Info.Profile, projectile.View);
